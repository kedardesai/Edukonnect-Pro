...
+ (NSDictionary *)loginUserWith:(EKPStudent *)student andType:(NSString *)userType
{
    EKPUser *currentUser = [EKPSingleton loadUser];
    //Web Service Call
    NSString *urlString = [NSString stringWithFormat:@"%@%@schoolcode=%@&type=%@&username=%@&password=%@&deviceid=%@", BASE_API_URL, SPECIAL_LOGIN_API_URL, student.studentSchoolCode, [[EKPSingleton loadUserRole] lowercaseString], student.studentUsername, student.studentPassword, currentUser.userDeviceId];
    NSURL *url = [NSURL URLWithString:urlString];
    NSMutableURLRequest *theRequest = [NSMutableURLRequest requestWithURL:url];
    [theRequest addValue: @"text/xml; charset=utf-8" forHTTPHeaderField:@"Content-Type"];
    [theRequest setHTTPMethod:@"GET"];
